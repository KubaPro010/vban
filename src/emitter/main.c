...
void usage()
{
    ...
    printf("-q, --quality=ID        : network quality indicator from 0 (low latency) to ∞ (latency will be higher and instability may occur of the system).\n");
...
int get_options(struct config_t* config, int argc, char* const* argv)
{
    ...
    while (1)
    {
        c = getopt_long(argc, argv, "i:p:s:b:q:d:r:n:f:c:l:h", options, 0);
        if (c == -1)
            break;
